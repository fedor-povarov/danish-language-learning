...
@Route(value = "verb/learning", layout = MainLayout.class)
@PageTitle("Verb Learning")
@CssImport("./styles/shared-styles.css")
@CssImport(value = "./styles/vaadin-text-field-styles.css", themeFor = "vaadin-text-field")
public class VerbLearningView extends VerticalLayout {

    private final FormLayout formLayout = new FormLayout();
    private final H1 header = new H1("Verb Learning");
    ...
    private Verb requiredVerb;

    public VerbLearningView(VerbService verbService) {
    ...
    private void prepareUiButtons() {
        nextSimpleWord.addClickListener(click -> {
            resetUiFields();
            requiredVerb = verbService.getVerbToLearn();
            englishWord.setValue(requiredVerb.getEnglish());
        });
        nextSimpleWord.addClickShortcut(Key.BACKSLASH);

        checkSimpleWord.addClickListener(click -> {
            Verb suggestedVerb = new Verb();
            suggestedVerb.setDanish(suggestedDanishWord.getValue());
            checkVerb(requiredVerb, suggestedVerb);
        });
    ...
    private void checkVerb(Verb requiredVerb, Verb suggestedVerb) {
        boolean isDanishWordCorrect = StringUtils.equals(requiredVerb.getDanish(), suggestedVerb.getDanish());

        actualDanishWord.setValue(requiredVerb.getDanish());
